...
newLevel p n fmls temp = 
  let
    (fr, temp') = Frame.newFrame n fmls temp
    (u, temp'') = Temp.newNum temp'
  in
   (Level{parent=p ,name=n, formals=fmls, frame=fr , uniq=u}, temp'')

allocLocal :: Level -> Bool -> Temp.Temp -> (Access, Level, Temp.Temp)
allocLocal lv@Level{frame=fr} escapes temp =
  let
    (ac, frame', temp') = Frame.allocLocal fr escapes temp
    level' = lv{frame=frame'}
  in
   (Access{level=level', access=ac}, level', temp')
   
allocLocal Outermost _ _ = error "fatal: allocLocal Outermost _ _"
...
ifThen e1 e2 temp = 
  let
    genstm = unCx e1
    (s, temp') = unNx temp e2
    (t, temp2) = Temp.newLabel temp'
    (f, temp3) = Temp.newLabel temp2
...
arrayExp size i temp =
  callExp (Temp.namedLabel "_initArray") [size, i] temp
...
acc_formals lv =
    fmap 
    (\a -> Access{level=lv, access=a})
    (Frame.formals $ frame lv)

fpExp :: Level -> Exp
fpExp lv = 
   Ex $ T.TEMP $ Frame.fp $ frame lv
