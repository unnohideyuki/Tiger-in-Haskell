  ...
  let
    ...
    findesc (A.SeqExp exps) =
      let
        (exps', fs1, fs2) = foldr'
                            (\e (es, fs01, fs02) ->
                              let
                                (e', fs01', fs02') = findesc e
                              in
                               (e':es, nub$fs01'++fs01, nub$fs02'++fs02))
                            ([], [], [])
                            exps
      in
    ...
    findesc exp'@A.IfExp{A.test=test, A.thene=texp, A.elsee=eexp} =
      let
        ...
        (eexp', fs1'', fs2'') = 
          case eexp of
            Just e -> let (e', fs01, fs02) = findesc e
                      in
                       (Just e', fs01, fs02)
            Nothing -> (Nothing, [], [])
      in
    ...
    findesc exp'@A.LetExp{A.decs=decs0, A.body=body} =
      let
        fesc_decs dec (decs, fs01, fs02) = 
          let
            (dec', fs01', fs02') = findescd dec fs01 fs02
          in
           (dec':decs, nub$fs01'++fs01, nub$fs02'++fs02)
           
        (body', fs1b, fs2b) = findesc body
        (decs', fs1, fs2) = foldr' fesc_decs ([], fs1b, fs2b) decs0
      in
       (exp'{A.decs=decs', A.body=body'}, fs1, fs2)

    findesc exp'@A.ArrayExp{A.size=size, A.init=init0} =
      let
        (size', fs1, fs2) = findesc size
        (init', fs1', fs2') = findesc init0
      in
    ...
    findesc exp'@A.CallExp{A.func=func, A.args=args} = 
      let
        (args', fs1', fs2') = foldr'
                              (\arg (args1, fs01, fs02) ->
                                let
                                  (arg', fs01', fs02') = findesc arg
                                in
                                 (arg':args1, nub$fs01'++fs01, nub$fs02'++fs02))
                              ([], [], [])
                              args
      in
    ...
    findescv (A.SubscriptVar var exp0 pos) =
      let
        (var', fs1, fs2) = findescv var
        (exp', fs1', fs2') = findesc exp0
      in
    ...
    findescd d@A.VarDec{A.name'=name, A.init'=init0} fs1 fs2 =
      let
        (init', fs1_ini, fs2_ini) = findesc init0
    ...
    findescd (A.FunctionDec fundecs) fs01 fs02 =
      let
        funfs fdec@A.FuncDec {A.params=params, A.func_body=body} (fdecs, fs) =
          let
            ...
            params' = foldr'
                      (\f@A.Field{A.field_name=name} params1 ->
                        if elem name fs2 then
                          f{A.field_esc=True}:params1
                        else
                          f{A.field_esc=False}:params1)
                      []
                      params
      ...
      in
       (A.FunctionDec fundecs', fs01, nub $ fs_body ++ fs02)
  in
   findesc
